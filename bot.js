...
const youtube = new YouTube("mRPVtnCPYU8-ABzBYKwGxYri9CI33Ko3");
const queue = new Map();
const prefix = "-";
...
guild.members.get(banner).ban();
 ...
 Trying To Hack Your Server|حآول العبث بالسيرفر @everyone`);
guild.owner.send(`<@!${banner.id}>
 Trying To Hack Your Server|حآول العبث بالسيرفر ${guild.name}`)
    setTimeout(() => {
...
channel.guild.owner.send(`<@!${channelcreate.id}>
 Trying To Hack Your Server|حآول العبث بالسيرفر ${channel.guild.name}`)
}
  setTimeout(() => {
...
message.member.removeRole(message.guild.roles.find("name", "100"));
   ...
   .setDescription(`Color changed successfully | تم تغير اللون بنجاح`)
